...
class ViewProductDivertController extends ViewBaseController
{
	...
	{
		$param = $request->param();
		$system = \addons\product_divert\model\productDivertModel::getSystemConfig();
		try {
			\addons\product_divert\model\productDivertModel::checkRealnameRequired($param["uid"]);
		} catch (\Exception $e) {
	...
	public function pushpulllist(\think\Request $request)
	{
		...
		$param = $request->param();
		$product_divert = \think\Db::name("product_divert")->field("id,push_invoice_id,pull_invoice_id,product_name,product_domain,product_ip,push_userid,pull_userid,push_username,pull_username,push_cost,pull_cost,status,create_time,end_time")->whereOr(["push_userid" => $param["uid"], "pull_userid" => $param["uid"]])->withAttr("create_time", function ($value) {
		...
		$data["Count"] = count($product_divert);
		$data["Limit"] = $param["limit"] ?? config("limit");
		$data["Pages"] = "";

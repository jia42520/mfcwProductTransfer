...
class productDivertModel
{
    public static function getSystemConfig()
    {
        ...
        $system = !empty($res["config"]) ? json_decode($res["config"], true) : [];
        if (!is_array($system)) {
            $system = [];
        }
        $defaults = [];
        ...
        $system = array_merge($defaults, $system);
        $system["product_range"] = $system["product_range"] ?? [];
        if (!is_array($system["product_range"])) {
            $system["product_range"] = [];
        }
        return $system;
    }
    public static function checkRealnameRequired($uid)
    {
        $system = self::getSystemConfig();
        if (!empty($system["realname_required"]) && !checkCertify($uid)) {
            $message = lang("PRODUCT_DIVERT_REALNAME_REQUIRED");
            if ($message == "PRODUCT_DIVERT_REALNAME_REQUIRED") {
                $message = "请先完成实名认证后再进行产品转移";
            }
            throw new \think\Exception($message);
        }
        return true;
    }
    ...
    {
        $system = self::getSystemConfig();
        self::checkRealnameRequired($param["uid"]);
        ...
        if (empty($product)) {
            throw new \think\Exception(lang("ABNORMA_DATA"));
        }
